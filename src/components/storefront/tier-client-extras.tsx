...
import { Award, Smartphone, User } from "lucide-react";
  ...
  if (tier === "starter") {
    return (
      <div className="border-b border-white/5 bg-brand-anthracite/30 px-4 py-2 text-center text-xs text-brand-cream/50">
        ...
        essentielle · sans compte client
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-wrap items-center justify-center gap-2 border-b border-white/5 bg-brand-anthracite/40 px-4 py-2 sm:justify-between sm:px-6">
      <div className="flex flex-wrap items-center gap-2">
        ...
        {features.loyalty && (
          <span className="inline-flex items-center gap-1.5 rounded-full border border-brand-gold/30 bg-brand-gold/10 px-3 py-1 text-xs font-medium text-brand-gold">
