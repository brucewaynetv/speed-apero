...
function getDefaultSelections(product: CatalogProduct): Record<string, string[]> {
  ...
  product.optionGroups?.forEach((group) => {
    const defaultOpt = group.options.find((o) => o.isDefault && !o.unavailable);
    if (defaultOpt) {
      defaults[group.name] = [defaultOpt.name];
    } else if (group.required) {
      const first = group.options.find((o) => !o.unavailable);
      defaults[group.name] = first ? [first.name] : [];
    } else {
      defaults[group.name] = [];
    }
  });
  ...
  return (
    <>
      <div className="relative aspect-video w-full overflow-hidden rounded-t-2xl">
        <Image
          src={getProductImage(product, imageOverride)}
          alt={product.name}
          fill
      ...
      <div className="space-y-4 p-5">
        <div>
          {product.badge ? <Badge className="mb-2">{product.badge}</Badge> : null}
          <h2 className="font-display text-3xl tracking-wide text-brand-cream">
            {product.name.toUpperCase()}
          </h2>
          <p className="mt-2 whitespace-pre-line text-sm text-brand-cream/70">
            {product.description}
          </p>
        </div>
        ...
        {product.optionGroups?.map((group) => (
          <div key={group.name}>
            <h3 className="mb-2 font-accent text-sm font-bold uppercase tracking-wider text-brand-orange">
              {group.name}
              {group.required ? <span className="text-brand-red"> *</span> : null}
            </h3>
            <div className="flex flex-wrap gap-2">
              {group.options.map((opt) => {
                const isSelected = selections[group.name]?.includes(opt.name);
                const disabled = Boolean(opt.unavailable);
                return (
                  <button
                    key={opt.name}
                    type="button"
